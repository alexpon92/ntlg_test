...
class MoviesTop(Base):
    ...
    def __repr__(self):
        return "<MoviesTop(movieid='%s', ratings_num='%s', ratings_avg='%s')>" % (self.movieid, self.ratings_num, self.ratings_avg)
...
top_rated_query = session.query(MoviesTop).filter(MoviesTop.ratings_num > 15)\
    .filter(MoviesTop.ratings_avg > 3.5)\
    .order_by(MoviesTop.ratings_avg.asc())
